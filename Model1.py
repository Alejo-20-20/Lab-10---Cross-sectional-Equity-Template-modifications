...
#from quantopian.pipeline.factors import TrueRange
...
def make_pipeline():
    ...
    holdrecommendations = SimpleMovingAverage(
        inputs=[ConsensusRecommendations.hold],
        window_length=10,
    )
    
    buyrecommendations = SimpleMovingAverage(
        inputs=[ConsensusRecommendations.buy],
        window_length=10,
    )
    
    sellrecommendations = SimpleMovingAverage(
        inputs=[ConsensusRecommendations.sell],
        window_length=10,
    )
    
    totalrecommendations = SimpleMovingAverage(
        inputs=[ConsensusRecommendations.total],
        window_length=10,
    )
    
    highequity = SimpleMovingAverage(
        inputs=[EquityPricing.high],
        window_length=10,
    )
    
    lowequity = SimpleMovingAverage(
        inputs=[EquityPricing.low],
        window_length=10,
    )
    
    #trueequity = TrueRange(
    ...
    holdrecommendations_winsorized = holdrecommendations.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    buyrecommendations_winsorized = buyrecommendations.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    sellrecommendations_winsorized = sellrecommendations.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    totalrecommendations_winsorized = totalrecommendations.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    highequity_winsorized = highequity.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    lowequity_winsorized = lowequity.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    #trueequity_winsorized = trueequity.winsorize(
        #min_percentile=0.05,
        #max_percentile=0.95)

    # Here we combine our winsorized factors, z-scoring them to equalize their influence
    combined_factor = (
        highequity_winsorized.zscore() + 
        lowequity_winsorized.zscore() +
        totalrecommendations_winsorized.zscore() + 
        sellrecommendations_winsorized.zscore() + 
        buyrecommendations_winsorized.zscore() +
        holdrecommendations_winsorized.zscore()
    )
