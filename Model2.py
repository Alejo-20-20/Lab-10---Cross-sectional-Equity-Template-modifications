...
def make_pipeline():
    ...
    high = SimpleMovingAverage(
        inputs=[EquityPricing.high],
        window_length=10
    )
    
    low = SimpleMovingAverage(
    ...
    enterprise_winsorized = enterprise.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    marketcap_winsorized = marketcap.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    earnings_winsorized = earnings.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    equitymedia_winsorized = equitymedia.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    high_winsorized = high.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    low_winsorized = low.winsorize(
        min_percentile=0.05,
        max_percentile=0.95)
    ...
    combined_factor = (
        enterprise_winsorized.zscore() + 
        marketcap_winsorized.zscore() +
        earnings_winsorized.zscore() +
        high_winsorized.zscore() +
        low_winsorized.zscore() +
